...
#         try:
    ...
    def get_opinion_orderbook(self, token_id: str, depth: int = 5, max_retries: int = 3) -> Optional[OrderBookSnapshot]:
            ...
            try:
                ...
                if response.errno != 0:
                    if attempt < max_retries - 1:
                        print(f"⚠️ Opinion API 返回错误码 {response.errno}, 第 {attempt + 1}/{max_retries} 次尝试")
    ...
    def get_polymarket_orderbook(self, token_id: str, depth: int = 5, max_retries: int = 1) -> Optional[OrderBookSnapshot]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not book:
                    if attempt < max_retries - 1:
                        print(f"⚠️ Polymarket 返回空订单簿, 第 {attempt + 1}/{max_retries} 次尝试")
                        time.sleep(retry_delay)
                        continue
                    return None
    ...
    def get_orderbooks_concurrent(self, matches: List[MarketMatch]) -> Dict[str, Tuple[Optional[OrderBookSnapshot], Optional[OrderBookSnapshot]]]:
        """并发获取所有市场的订单簿
        ...
        Returns:
            字典，key 为市场索引，value 为 (opinion_yes_book, poly_yes_book) 元组
        """
        ...
        def fetch_orderbook_pair(idx: int, match: MarketMatch) -> Tuple[int, Optional[OrderBookSnapshot], Optional[OrderBookSnapshot]]:
            """获取单个市场的订单簿对"""
            try:
                ...
                return (idx, opinion_yes_book, poly_yes_book)
            except Exception as e:
                print(f"⚠️ 获取订单簿失败 [{idx}]: {e}")
                return (idx, None, None)
        
        # 使用线程池并发获取
        max_workers = int(os.getenv("ORDERBOOK_WORKERS", "10"))  # 可通过环境变量配置并发数
        task_timeout = float(os.getenv("ORDERBOOK_TASK_TIMEOUT", "10.0"))  # 单个任务超时（秒）
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            futures = {executor.submit(fetch_orderbook_pair, idx, match): idx for idx, match in enumerate(matches)}
            
            # 收集结果（带超时和进度显示）
            for future in as_completed(futures, timeout=task_timeout * len(matches) / max_workers + 60):
                try:
                    # 为每个 future 设置超时
                    idx, opinion_yes_book, poly_yes_book = future.result(timeout=task_timeout)
                    results[idx] = (opinion_yes_book, poly_yes_book)
                    completed_count += 1
                    ...
                    idx = futures[future]
                    print(f"⏱️ 任务超时 [{idx}]: 超过 {task_timeout}s")
                    results[idx] = (None, None)
                    completed_count += 1
                except Exception as e:
                    idx = futures.get(future, '?')
                    print(f"⚠️ 并发任务异常 [{idx}]: {e}")
    ...
    def execute_arbitrage_pro(self, non_interactive: bool = False):
        ...
        completed_count = 0
        max_workers = int(os.getenv("ORDERBOOK_WORKERS", "10"))
        ...
        if strategy['type'] == 'immediate':
            ...
            try:
                ...
                if strategy['first_platform'] == 'opinion':
                    ...
                    success, result1 = self._place_opinion_order_with_retries(
                        order1,
                        context="立即套利首单"
                    )
                    if not success or not result1:
                        print(f"❌ {strategy['first_platform'].upper()} 下单失败（已尝试 {self.order_max_retries} 次）")
                        return
                    ...
                    success, result1 = self._place_polymarket_order_with_retries(
                        order1,
                        OrderType.GTC,
                        context="立即套利首单"
                    )
                ...
                if strategy['second_platform'] == 'opinion':
                    ...
                    success, result2 = self._place_opinion_order_with_retries(
                        order2,
                        context="立即套利对冲单"
                    )
                    if not success or not result2:
                        print(f"❌ {strategy['second_platform'].upper()} 下单失败（已尝试 {self.order_max_retries} 次）")
                        return
                    ...
                    success, result2 = self._place_polymarket_order_with_retries(
                        order2,
                        OrderType.GTC,
                        context="立即套利对冲单"
                    )
        ...
        try:
            ...
            if strategy['first_platform'] == 'opinion':
                ...
                if not success or not result:
                    print(f"❌ 下单失败（已尝试 {self.order_max_retries} 次）")
                    return
                ...
                success, result = self._place_polymarket_order_with_retries(
                    order,
                    OrderType.GTC,
                    context="潜在套利首单"
                )
                if not success or not result:
                    print(f"❌ 下单失败（已尝试 {self.order_max_retries} 次）")
                    return
                first_order_id = result.get('orderID') if isinstance(result, dict) else None
                if not first_order_id and isinstance(result, dict):
                    first_order_id = result.get('order_id')
                print(f"✅ 订单已提交，订单 ID: {first_order_id}")
                            ...
                            with monitoring_lock:
                                ...
                                try:
                                    ...
                                    if strategy['first_platform'] == 'opinion':
                                        ...
                                        if not success or not result:
                                            print(f"❌ 重新挂单失败（已尝试 {self.order_max_retries} 次）")
                                            stop_monitoring.set()
                                            break
                                        first_order_id = result.result.order_data.order_id
                                ...
                                if filled > filled_amount[0]:
                                    ...
                                    try:
                                        if strategy['second_platform'] == 'opinion':
                                            ...
                                            success, hedge_result = self._place_opinion_order_with_retries(
                                                hedge_order,
                                                context="监控对冲"
                                            )
                                            if success and hedge_result:
                                                print("✅ Opinion 对冲订单已提交")
                                            else:
                                                print(f"❌ Opinion 对冲失败（已尝试 {self.order_max_retries} 次）")
                                        else:
                                            ...
                                            success, hedge_result = self._place_polymarket_order_with_retries(
                                                hedge_order,
                                                OrderType.GTC,
                                                context="监控对冲"
                                            )
                                            if success:
                                                print("✅ Polymarket 对冲订单已提交")
                                            else:
                                                print(f"❌ Polymarket 对冲失败（已尝试 {self.order_max_retries} 次）")
