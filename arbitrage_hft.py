...
try:
    ...
    print("⚠️  未安装orjson，使用标准json库。建议: pip install orjson")
...
try:
    ...
    print("⚠️  未安装aiohttp，异步I/O优化将受限。建议: pip install aiohttp")
...
import requests
from py_clob_client.clob_types import OpenOrderParams, BookParams
...
class CrossPlatformArbitrage:
    ...
    def __init__(self):
        ...
        try:
            self.orderbook_batch_size = max(1, int(os.getenv("ORDERBOOK_BATCH_SIZE", "50")))
        except Exception:
            self.orderbook_batch_size = 50  # 从20提升到50
        try:
            self.polymarket_books_chunk = max(1, int(os.getenv("POLYMARKET_BOOKS_BATCH", "100")))
        except Exception:
            self.polymarket_books_chunk = 100  # 从25提升到100
        try:
            self.opinion_orderbook_workers = max(1, int(os.getenv("OPINION_ORDERBOOK_WORKERS", "20")))
        except Exception:
            self.opinion_orderbook_workers = 20  # 从5提升到20
        try:
    ...
    def calculate_opinion_fee_rate(price: float) -> float:
        ...
        优化: 使用LRU缓存 + staticmethod避免重复计算和self引用开销
    ...
    def calculate_opinion_adjusted_amount(self, price: float, target_amount: float) -> float:
        ...
        if Fee_provisional > 0.5:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"💰 Opinion 手续费计算: price={price:.3f}, fee_rate={fee_rate:.6f}, "
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"   目标数量: {target_amount:.2f} -> 修正后下单数量: {A_order:.2f}")
        return A_order
    ...
    def calculate_opinion_effective_amount(self, price: float, order_amount: float) -> float:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"💰 Opinion 实际数量计算: 订单数量={order_amount:.2f}, "
    ...
    def _throttle_opinion_request(self) -> None:
        ...
        with self._opinion_rate_lock:
            ...
            if wait > 0:
                # 在锁内直接sleep，避免重复检查
                time.sleep(wait)
            self._opinion_last_request = time.perf_counter()
    ...
    def _fetch_opinion_orderbooks_parallel(
        ...
        优化: 如果启用异步模式，使用asyncio提升性能
        """
        # 尝试使用异步版本（如果可用）
        use_async = os.getenv("USE_ASYNC_ORDERBOOK", "1") == "1"
        if use_async and AIOHTTP_AVAILABLE:
            try:
                return self._fetch_opinion_orderbooks_async(token_ids, depth)
            except Exception as e:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"异步获取失败，回退到线程池: {e}")
    ...
    def _fetch_opinion_orderbooks_async(
        self,
        token_ids: List[str],
        depth: int = 5,
    ) -> Dict[str, Optional[OrderBookSnapshot]]:
        """异步批量获取Opinion订单簿（高性能版本）

        优化: 使用asyncio.run在同步环境中运行异步代码
        """
        tokens = self._dedupe_tokens(token_ids)
        if not tokens:
            return {}
        ...
        # 在同步函数中运行异步代码
        return asyncio.run(self._fetch_opinion_orderbooks_async_impl(tokens, depth))

    async def _fetch_opinion_orderbooks_async_impl(
        self,
        tokens: List[str],
        depth: int = 5,
    ) -> Dict[str, Optional[OrderBookSnapshot]]:
        ...
        优化: 使用asyncio.gather并发执行所有请求，比ThreadPoolExecutor更高效
        """
        async def fetch_one(token: str) -> tuple[str, Optional[OrderBookSnapshot]]:
            """获取单个订单簿的异步包装"""
            # 使用run_in_executor在异步环境中运行同步代码
            loop = asyncio.get_event_loop()
            try:
                # 限流
                await loop.run_in_executor(None, self._throttle_opinion_request)
                # 获取订单簿
                snapshot = await loop.run_in_executor(
                    None,
                    self.get_opinion_orderbook,
                    token,
                    depth
                )
        ...
        # 并发获取所有订单簿
        results = await asyncio.gather(*[fetch_one(token) for token in tokens])

        # 转换为字典
        return {token: snapshot for token, snapshot in results}
    ...
    def get_polymarket_orderbooks_bulk(
        ...
        优化: 使用异步模式并发处理多个chunk
        """
        # 尝试使用异步版本
        use_async = os.getenv("USE_ASYNC_ORDERBOOK", "1") == "1"
        if use_async and AIOHTTP_AVAILABLE:
            try:
                return self._get_polymarket_orderbooks_async(token_ids, depth, max_retries)
            except Exception as e:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"异步获取Polymarket订单簿失败，回退到同步: {e}")
        ...
        chunk_size = max(1, getattr(self, "polymarket_books_chunk", 100))
    ...
    def _get_polymarket_orderbooks_async(
        self,
        token_ids: List[str],
        depth: int = 5,
        max_retries: int = 2,
        ...
        优化: 使用asyncio并发处理多个chunk，大幅提升吞吐量
        """
        tokens = self._dedupe_tokens(token_ids)
        if not tokens:
            return {}

        return asyncio.run(self._get_polymarket_orderbooks_async_impl(tokens, depth, max_retries))

    async def _get_polymarket_orderbooks_async_impl(
        self,
        tokens: List[str],
        depth: int = 5,
        max_retries: int = 2,
    ) -> Dict[str, OrderBookSnapshot]:
        """异步获取Polymarket订单簿的实际实现"""
        chunk_size = max(1, getattr(self, "polymarket_books_chunk", 100))
        chunks = [tokens[i:i + chunk_size] for i in range(0, len(tokens), chunk_size)]

        async def fetch_chunk(chunk: List[str]) -> Dict[str, OrderBookSnapshot]:
            """异步获取一个chunk的订单簿"""
            loop = asyncio.get_event_loop()
            try:
                params = [BookParams(token_id=tid) for tid in chunk]
                books = await loop.run_in_executor(
                    None,
                    self.polymarket_client.get_order_books,
                    params
                )
                ...
                if books:
                    for idx, book in enumerate(books):
                        ...
                        bids = self._normalize_polymarket_levels(getattr(book, "bids", []), depth, reverse=True)
                        asks = self._normalize_polymarket_levels(getattr(book, "asks", []), depth, reverse=False)
                        chunk_snapshots[token_key] = OrderBookSnapshot(
                            bids=bids,
                            asks=asks,
                            source="polymarket",
                            token_id=token_key,
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"⚠️ 批量获取Polymarket订单簿chunk失败: {exc}")
                return {}
        ...
        for chunk_result in results:
            all_snapshots.update(chunk_result)
    ...
    def _derive_no_orderbook(self, yes_book: OrderBookSnapshot, no_token_id: str) -> OrderBookSnapshot:
        ...
        优化: 使用列表推导 + sorted一次性完成，避免append和二次排序
        """
        if not yes_book:
            return None

        # NO的bids来自YES的asks (YES卖=NO买)
        # 使用列表推导 + sorted，一次性完成过滤、转换和排序
        no_bids = sorted(
            [OrderBookLevel(price=self._round_price(1.0 - level.price), size=level.size)
             for level in yes_book.asks
             if level.price is not None and level.size is not None],
            key=lambda x: x.price,
            reverse=True  # bids从高到低
        )
    ...
    def execute_arbitrage_pro(self):
        ...
        for batch_start in range(0, total_matches, batch_size):
            ...
            # 优化: 增加并发worker数，从2提升到4
            with ThreadPoolExecutor(max_workers=4) as batching_executor:
