...
class CrossPlatformArbitrage:
    ...
    def search_polymarket_market(self, query: str, rules_text: str = "", debug: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            best_match = None
            best_similarity = 0.0
            rules_lower = (rules_text or "").lower().strip()

            for market in markets:
                market_description = (market.get("description") or event_description or "").lower().strip()
                similarity = self._calculate_similarity(rules_lower, market_description)

                if similarity >= 0.95 or (rules_lower and rules_lower == market_description):
    ...
    def match_markets_by_search(self, topic_types: Optional[List[str]] = None) -> List[MarketMatch]:
        ...
        if process_binary:
            ...
            for i, op_market in enumerate(binary_markets, 1):
                ...
                # 只在第一次调用时启用调试
                pm_market = self.search_polymarket_market(
                    query=op_title,
                    rules_text=op_market.get("rules", ""),
                    debug=True,
                )
