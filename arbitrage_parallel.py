...
#         try:
    ...
    def execute_arbitrage_pro(self, non_interactive: bool = False):
        ...
        # 并发获取所有订单簿
        print(f"🚀 开始并发获取 {len(self.market_matches)} 个市场的订单簿...")
        start_time = time.time()
        orderbook_results = self.get_orderbooks_concurrent(self.market_matches)
        elapsed = time.time() - start_time
        print(f"✅ 订单簿获取完成，耗时 {elapsed:.2f}s (平均 {elapsed/len(self.market_matches):.3f}s/市场)\n")
        
        # 扫描所有市场，收集立即套利和潜在套利
        immediate_opportunities = []  # ask + ask (立即执行)
        pending_opportunities = []    # bid + ask (等待成交)
        
        for idx, match in enumerate(self.market_matches):
            print(f"[{idx+1}/{len(self.market_matches)}] 扫描: {match.question[:70]}...")
            
            # 从并发获取的结果中取出订单簿
            opinion_yes_book, poly_yes_book = orderbook_results.get(idx, (None, None))
            ...
            if opinion_yes_book and opinion_yes_book.asks and poly_no_book and poly_no_book.asks:
                ...
                if op_yes_ask and pm_no_ask and op_yes_ask.price is not None and pm_no_ask.price is not None:
                    ...
                    if cost < THRESHOLD_PRICE and min_size > THRESHOLD_SIZE:
                        ...
                        if self.immediate_exec_enabled:
                            if self.immediate_min_percent <= profit_rate <= self.immediate_max_percent:
                                ...
                                print(f"  🔶 利润率 {profit_rate:.2f}% 不在阈值范围 [{self.immediate_min_percent:.2f}%,{self.immediate_max_percent:.2f}%]，跳过自动执行")
